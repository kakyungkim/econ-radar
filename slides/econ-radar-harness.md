...
- **사실 기준선** <span class="tag new">NEW</span> : `fact-checker`가 핵심 수치 최대 10개를 원문과 대조한다. 출처 없는 수치는 막고, 단일 소스는 표기하며, 1차 출처(IR·규제기관)를 명시한다.
...
- **왜 저녁 18:30인가** : 한국 장 마감 뒤라 그날 국내 시장을 온전히 담는다. 미국 장은 아침 수집이 유리하지만, 국내 시장 반영을 앞세워 저녁을 골랐다.
- **발행 채널 셋** : 블로그(아카이브), 텔레그램(@econradar), **이메일** <span class="tag new">NEW</span>(Buttondown)로 내보내고, 트리거는 모두 git push 감지(GitHub Actions)로 잡는다.
- **저녁에 동시 발송** : 같은 git push가 블로그 배포, 텔레그램, 이메일 워크플로를 한꺼번에 깨워서 세 채널이 거의 동시에 도착한다.
- **승인 게이트 폐기** : 처음엔 발행 전에 사람이 승인했지만 이틀 만에 접었다. "확인 전까지 미발송"에서 **"발송하되 이상이 확인되면 발송 취소"**로 바꿨다.
- **vault 전체 추적** : private git에 통째 담아, 클라우드가 과거를 읽고 여러 주에 걸친 흐름을 잇는다.
