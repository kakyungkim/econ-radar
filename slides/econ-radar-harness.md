...
1. **라이브 발행본** : 오늘자 블로그 리포트를 열어 목차로 점프하고, 영어 단어 발음을 재생한다.
2. **옵시디언 그래프와 `cat`** : vault를 옵시디언으로 열면 주제 MOC와 데일리가 엮인 그래프가 뜬다. 터미널에서 `cat vault/topics/AI.md`를 쳐 보면 "그냥 마크다운"이다.
3. **LLM이 vault를 읽는다** : "최근 2주 GLP-1 흐름 정리해줘"를 vault만 보고 답하게 한다. 연결된 자산이 곧 물어볼 수 있는 지식이다.

<span class="small">라이브: 블로그 kakyungkim.github.io/econ-radar, 텔레그램 @econradar, 이메일 받은편지함</span>
