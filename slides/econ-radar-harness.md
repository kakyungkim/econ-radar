...
<span class="small">스터디 발표 · 2026-06-27 (v4) · 블로그 · 텔레그램 · 이메일 3채널</span>
...
<li><span class="d">Part 2</span><strong>어떻게 — 구조</strong> — 8명의 에이전트 팀 · 설계 4원칙 · 파일로 둔 기준선 · 자동화 3채널</li>
<li><span class="d">Part 3</span><strong>진화</strong> — 17일간 운영·배포가 설계를 고친 기록 + <em>프로덕션이 가르친 것</em></li>
<li><span class="d">Part 4</span><strong>원칙·교훈·토의</strong> — 안전선 · 하네스로 배운 점 · 로드맵 · 데모</li>
</ul>

<br>

<span class="small">슬라이드는 선형으로 넘기되, Q&amp;A 때 "Part N"으로 돌아옵니다. 각 Part 앞에 구분 슬라이드가 있습니다.</span>
...
- **발행 채널 3개** — 블로그(아카이브)·텔레그램(@econradar, 저녁 즉시)·**이메일** <span class="tag new">NEW</span>(Buttondown). 트리거는 전부 git push 감지(GitHub Actions).
- **생성 시각 ≠ 배달 시각** — 이메일은 저녁에 생성하되 **다음날 아침 7시 예약 발송**(장 개장 전 받아보게). 텔레그램은 저녁 즉시.
- 처음엔 발행 전 사람 승인이었지만 **이틀 만에 폐기** — "확인 전까지 안 나간다" → **"나가되, 이상하면 내린다"**. 이메일만은 첫 단계가 **초안(draft)** 후 확인.
...
<ul class="tl">
<li><span class="d">6/14</span>미뤄둔 백로그 처리 — MOC 갱신 규칙·렌즈 범위 분리·승격 기준 문서화</li>
<li><span class="d">6/15</span>텔레그램 직접 발송이 네트워크에 막힘 → <strong>GitHub Actions(push 감지)로 발송 이관</strong></li>
<li><span class="d">6/16–18</span>Actions 다중커밋 감지 버그 픽스 + <strong>뉴스 신선도·중복 방지</strong> — 같은 약·기업이 며칠 반복되는 "에이전트 관성" 제거</li>
<li><span class="d">6/24</span>옵시디언 → <strong>OKF 포맷 마이그레이션</strong> — 위키링크 1,228개 변환·frontmatter·index/log</li>
<li><span class="d">6/25</span><strong>이메일 채널</strong> 추가(Buttondown, 다음날 아침 예약) + 발음·버튼·섹션 목차 등 프로덕션 다듬기</li>
</ul>

> 1주차 패턴이 그대로 이어진다 — *운영·배포가 새 실패 모드를 드러내고, 그게 규칙·게이트·채널로 바뀐다.*
...
| 증상 (어디서) | 진짜 원인 | 하네스 교훈 |
| --- | --- | --- |
| 이메일 버튼·글씨가 깨짐 | 메일은 JS·외부 CSS 안 됨, 클라이언트가 링크색을 덮어씀 | **"로컬에서 됨 ≠ 받은편지함에서 됨"** — 인라인 CSS·table·`!important` |
| 새 데일리가 옛 포맷으로 회귀 | OKF 규약이 `~/.claude` 전역에만 있어 **클라우드가 못 봄** | 규약·기준선은 **레포 안에** 둬야 지속된다 |
| 발음이 엉뚱한 단어 | 표제어 대신 괄호 속 영어 설명을 읽음 | **데이터→렌더 계약**을 명시(표제어 = 발음) |
| "고쳤는데 그대로" | 배포 워크플로 지연 + 브라우저·CDN 캐시 | 라이브는 **curl로 검증**, 캐시를 의심 |

> 채널마다 런타임이 다르다 — 같은 산출물도 **배달 환경에서 다시 검증**해야 한다.
...
- **운영이 설계를 고친다** — 17일간 거의 매일 구조가 바뀌었다. 전부 운영 중 불만에서 출발했다.
- **배달 환경마다 다시 검증한다** — 로컬 성공이 전부가 아니다. 이메일·클라우드·캐시는 각자 다른 런타임이다.
...
- **OKF 지속성** — 마이그레이션은 했지만 클라우드 에이전트가 아직 옛 포맷으로 생성 → 규약을 **레포 안으로** 이식해야 정착(현재 매일 클린업 필요).
- **이메일 자동 발송 전환** — 지금은 초안 검토 단계. 안정되면 레포 변수 하나로 다음날 아침 자동 발송.
- **중복 규칙 운용 검증** — "한 사건 한 번 서술" 규칙을 넣었으니 실제 발행본에서 중복감이 줄었는지 며칠 추적.

> 하네스는 끝나는 게 아니라 **백로그가 도는 것** — 비판이 곧 다음 할 일.
...
<span class="small">QR = 아카이브 · 블로그·텔레그램 @econradar·이메일 3채널 · 데모: 오늘자 HTML 리포트</span>
